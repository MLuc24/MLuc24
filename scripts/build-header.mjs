...
// Run from the repository root: node scripts/build-header.mjs
//
// The SVGs are the editable source, but the README points at assets/header-*.png.
// GitHub does not reliably display an SVG pulled from raw.githubusercontent.com, and a
// header that silently fails to load is worse than one that cannot animate. After editing
// this file, regenerate the SVGs and re-export both PNGs at 880x326, 2x scale, on the
// GitHub canvas colours (#FFFFFF light, #0D1117 dark).
